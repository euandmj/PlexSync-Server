...
class Server:
    ...
    def downloadTorrent(self, uri, pathIndex):
        ...
        def getAppropriateFilePath(torrent, pathIndex):
            ...
            try:
                ...
                # group4 - rest
                regex = re.match(
                    r'(.*?)\.S?(\d{1,2})E?(\d{2})\.(.*)', torrent['name'].replace(' ', '.'))
                ...
                if regex:
                    ...
                    # get all folders
                    dirs = [f for f in os.listdir(client_path) if not os.path.isfile(
                    ...
                    for d in dirs:
                        ...
                        if difflib.SequenceMatcher(None, a=media_name.lower(), b=d.lower()).ratio() >= 0.77:
                            ...
                            tv_dir = getSeasonSubDir(
                                media_name, regex.group(2), client_path + "\\" + d)
                            return tv_dir
                ...
                self.logger.log(
                    "ERROR getAppropiateFilePath: %s - %s" % (e, torrent["name"]))
        ...
        def overrideFilePath(hash):
            ...
            xc = xClient(self.config.get("qBittorrent", "host"), self.config.get(
                "qBittorrent", "username"), self.config.get("qBittorrent", "password"))
    ...
    def getTorrentList(self):
        ...
        try:
            ...
            for t in _t:
                torrents.append(
                    str(t["hash"]) + "~" + t["name"] + "~" + str(t["progress"]) + "~" + t["state"])
